...
const formatKey = (snake) => {
  ...
  const pretty = parts.map((p, idx) => {
    ...
    // Title Case words like 'processor', 'brand', 'series', etc.
...
const buildFilter = (item) => {
  ...
  if (processor_brand) raw.processor_brand = processor_brand;
  if (processor_series) raw.processor_series = processor_series;
  if (processor_model) raw.processor_model = processor_model;
  if (processor_generation) raw.processor_generation = processor_generation;
  if (ram) raw.ram = ram;
  if (storage_ssd) raw.storage_ssd = storage_ssd;
  if (storage_hdd) raw.storage_hdd = storage_hdd;
  if (display_size) raw.display_size = display_size;
  if (display_resolution) raw.display_resolution = display_resolution;
  if (graphics) raw.graphics = graphics;
  if (os) raw.os = os;
  if (touchscreen) raw.touchscreen = touchscreen;
  if (fingerprint) raw.fingerprint = fingerprint;
  if (color) raw.color = color;

  return toPresentationKeys(raw);
};
